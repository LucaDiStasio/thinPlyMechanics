...
def writeData(outdir,workbookname,data,boundaryCase):
    ...
    titleFormat = workbook.add_format({'bold': 1})
    subtitleFormat = workbook.add_format({'bold': 1})
    ...
    czWorksheet.write(1,0,'BOUNDARY CONDITIONS')
    czWorksheet.write(2,0,'NORTH')
    czWorksheet.write(2,1,'SOUTH')
    czWorksheet.write(2,2,'EAST')
    czWorksheet.write(2,3,'WEST')
    czWorksheet.write(2,4,'Vf [%]')
    czWorksheet.write(1,5,'Coefficients')
    czWorksheet.write(2,5,'A [%/deg]')
    czWorksheet.write(2,6,'B [%]')
    czWorksheet.write(1,7,'Covariance')
    czWorksheet.write(2,7,'cov(1,1)')
    czWorksheet.write(2,8,'cov(1,2)')
    czWorksheet.write(2,9,'cov(2,1)')
    czWorksheet.write(2,10,'cov(2,2)')
    czWorksheet.write(1,11,'std err')
    czWorksheet.write(2,11,'A [%/deg]')
    czWorksheet.write(2,12,'B [%]')
    Vf = [0.000079,0.0001,0.2,0.3,0.4,0.5,0.55,0.6,0.65]
    nVf = len(Vf)
    initVf = 3
    for c,case in enumerate(boundaryCase):
