...
\begin{equation}
\begin{split}
G_{I,r\theta} \sim&\frac{1}{2R_{f}\delta}\cos^{2}\left(\Delta\theta\right) \left(K_{xx}u^{2}_{x}\left(\Delta\theta\right)+K_{xy}u_{y}u_{x}+\widetilde{F}_{x}u_{x}\left(\Delta\theta\right)\right)+\\
