...
\node[anchor=west] at(\domlim+2.25*\l,0.5*\domlim+2*\l) {\huge$\color{blue}\checkmark$ \color{black}Espace 2D};
\node[anchor=west] at(\domlim+2.25*\l,0.5*\domlim+\l) {\huge$\color{blue}\checkmark$ \color{black}Solide \'elastique lin\'eaire};
\node[anchor=west] at(\domlim+2.25*\l,0.5*\domlim) {\huge$\color{blue}\checkmark$ \color{black}Pilot\'e en d\'eplacement};
\node[anchor=west] at(\domlim+2.25*\l,-0.5*\domlim) {\huge$\color{blue}\checkmark$ \color{black}Conditions aux limites de Dirichlet};
\node[anchor=west] at(\domlim+2.25*\l,-0.5*\domlim-\l) {\huge$\color{blue}\checkmark$ \color{black}M\'ecanique lin\'eaire \'elastique de la rupture};
\node[anchor=west] at(\domlim+2.25*\l,-0.5*\domlim-2*\l) {\huge$\color{blue}\checkmark$ \color{black}Interactions de contact};
