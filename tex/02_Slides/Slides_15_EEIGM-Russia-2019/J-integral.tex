...
\begin{tikzpicture}[scale=1.5,cap=round,x=1.5cm,y=1.5cm]
...
\draw[<->,line width=1.3mm,draw=red] (0,2.75) -- (2,2.75);
\node[anchor= south,text=red] at (1,2.75) {\Huge $\mathbf{\Delta a}$};
\node[anchor= north west,text=red] at (0.8,2.7) {\Huge \bf{crack closed}};
...
\node[text=blue,anchor=south west] at (1.35*\normalcos,-1.35*\normalsin) {\Huge $\mathbf{\Gamma_{i}}$};
...
\node[text=green,anchor=north west] at (2.5,0.7) {\Huge $\mathbf{\underline{n}_{i}^{num}}$};
