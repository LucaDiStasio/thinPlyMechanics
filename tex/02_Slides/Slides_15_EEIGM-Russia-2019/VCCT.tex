...
\draw[->,dashed, line width = 1.25mm] (-6.25,-2) -- (-6.25,2) ;
...
\draw[<->,line width=1.3mm,draw=red] (0,1.75) -- (2,1.75);
\node[anchor= south,text=red] at (1,1.75) {\Huge $\mathbf{\Delta a}$};
\node[anchor= north west,text=red] at (0.8,1.7) {\Huge \bf{crack closed}};
...
\draw[->,line width=1.75mm,draw=blue] (0,0) -- (-0.5,0);
\node[anchor= north,text=blue] at (-0.6,-0.1) {\Huge $\mathbf{X_{C}^{u}}$};
...
\node[anchor= south west,text=green] at (-1,-0.25+0.375) {\Huge $\mathbf{\Delta w_{C}}$};
