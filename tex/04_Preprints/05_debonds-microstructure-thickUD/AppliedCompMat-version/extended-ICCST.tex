...
To model the symmetric repetition of $n\times k-symm$ we adopt, on the upper boundary, conditions of coupling of the vertical displacements $u_{z}$ of the type

\begin{equation}\label{eq:symmcoupling}
u_{z}\left(x,h\right) = u^{\nu}_{z},
\end{equation}

where $h$ is the total height of the RVE defined in Eq.~\ref{eq:lengthheight} and $u^{\nu}_{z}$ is the constant value of the vertical displacement, equal for all the points on the upper boundary, which is \emph{a priori} unknown and is evaluated as part of the elastic solution.\\
