...
\begin{equation}
Q_{eff,lmij}^{LAM}=\frac{1}{h}\sum_{k=1}^{N}t_{k}T_{k,}^{-1}Q_{k,lmij}\left(\underline{\underline{T}}_{k}^{-1}\right)^{T}_{}\left(\frac{1}{2}\left(\delta_{ik}\delta_{jl}+\delta_{il}\delta_{jk}\right)+\tilde{\beta}_{k,ij}\left(\tilde{\varepsilon}_{ij}^{LAM}\right)^{-1}\right)
\end{equation}
