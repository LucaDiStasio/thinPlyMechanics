...
\begin{figure}[!h]
    \includegraphics[width=\textwidth]{paperB/RUC.pdf}
...
$R_{f}$ is the fiber radius, assumed to be equal to $1\ \mu m$, and $V_{f}$ the fiber volume fraction. Notice that the value of $1\ \mu m$ for the fiber radius is arbitrary and chosen for simplicity: in the context of Linear Elastic Fracture Mechanics (LEFM), the Energy Release Rate (ERR) is directly proportional to the fiber radius, thus evaluation of the ERR for another value of fiber radius requires simply a multiplication.\\
The RVE is made up by the repetition of the unit cell of Figure~\ref{chap3:paperB:fig:modelschem} $n$ times in the horizontal (loading) direction and $k$ times in the vertical (through-the-thickness) direction (see Figure~\ref{chap3:paperB:fig:thickply}). The fiber of the unit cell placed in the center of the RVE is always partially debonded, and the debond has an angular size $\Delta\theta$.
...
The RVE is symmetric with respect to the horizontal axis, thus only half of it is explicitly modelled and conditions of symmetry are applied on the lower boundary of the RVE. On the left and right boundaries, conditions of coupling of the $x$-displacement (horizontal displacement) are applied. This implies that the RVE 
