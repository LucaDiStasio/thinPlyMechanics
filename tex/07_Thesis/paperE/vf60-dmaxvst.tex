...
ylabel style={at={(axis description cs:-0.05,.5)},font=\fontsize{48}{40}\selectfont},
tick align=outside,
...
xlabel={$t_{90^{\circ}}\left[k\right]$},
ylabel={$\Delta\theta_{max}\left[^{\circ}\right]$},
% LEGEND ENTRIES
...
\addlegendentry{$n=11$, $G_{Ic}=2\ \left[\nicefrac{J}{m^{2}}\right]$, $\lambda=0.2$}

% S5, lambda = 0.35
\addplot[line width=\linewidth,mark size=\markersize,black,mark=square*,mark options={fill=white}]
table{
1 90
3 60
5 60
...
\addlegendentry{$n=11$, m=1, $G_{Ic}=2\ \left[\nicefrac{J}{m^{2}}\right]$, $\lambda=0.2$}
