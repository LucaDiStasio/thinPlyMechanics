...
\draw[->,black, line width = 5pt] (-4,0) -- (-4,2.5);
\draw[->,black, line width = 5pt] (-4,0) -- (-4,-2.5);
\node  at (-4,0) {\includegraphics[height=0.5\textheight]{0-90_2_symm}};
\draw[<->,red, line width = 1.25pt] (-4.3,-0.5) -- (-3.7,-0.625);
\node[red, below]  at (-4,-0.65) {\tiny$\mathbf{\sim 15\ mm}$};
\node[above, right]  at (-3.8,2.53) {\Huge$\mathbf{\varepsilon}$};
\node[below, left]  at (-4.2,-2.5) {\Huge$\mathbf{\varepsilon}$};

\draw[->,black, line width = 5pt] (-2,0) -- (-2,2.5);
\draw[->,black, line width = 5pt] (-2,0) -- (-2,-2.5);
\node  at (-2,0) {\includegraphics[height=0.5\textheight]{0-90_symm_edge}};
\draw[<->,red, line width = 1.25pt] (-2.4,-0.6) -- (-1.60,-0.6);
\node[red, below]  at (-2,-0.6) {\tiny$\mathbf{\sim 2\ mm}$};
\node[above, right]  at (-1.8,2.5) {\Huge$\mathbf{\varepsilon}$};
\node[below, left]  at (-2.2,-2.5) {\Huge$\mathbf{\varepsilon}$};

\draw[->,black, line width = 5pt] (1,0) -- (1,2.5);
\draw[->,black, line width = 5pt] (1,0) -- (1,-2.5);
\node  at (1,0) {\includegraphics[height=0.5\textheight,width=0.5\textheight,angle=90]{0-90_symm_debondetail}};
\draw[<->,red, line width = 1.25pt] (0.675,-0.45) -- (1.6,-0.45);
\node[red, below]  at (1.05,-0.475) {\tiny$\mathbf{\sim 8\ \mu m}$};
\node[above, right]  at (1.2,2.5) {\Huge$\mathbf{\varepsilon}$};
\node[below, left]  at (0.8,-2.5) {\Huge$\mathbf{\varepsilon}$};

\node[anchor=north]  at (-3.8,-2.75) {\large$\mathbf{\left[0,90_{2}\right]_{S}}$,};

\node[anchor=north]  at (-1.8,-2.75) {\large$\mathbf{\left[0,90\right]_{S}}$};

\node[anchor=north]  at (1.2,-2.75) {\large$\mathbf{\left[0,90\right]_{S}}$};
...
\begin{column}{0.5\textwidth}
\centering
\begin{alertblock}{\centering\small\bf New set of BC!}
\centering\small To represent debonds appearing on alternating sides along the vertical direction.
\end{alertblock}
\begin{equation*}
\begin{split}
\scriptsize
...
\begin{frame}
\frametitle{\vspace{0.2cm}\small Equivalent boundary conditions: anti-symmetric coupling, validation}
\vspace{-1.25cm}
\centering
\begin{columns}[c]
\begin{column}{0.5\textwidth}
...
\item Non-interpenetration of crack faces
\item Frictionless sliding of crack faces
\item $0^{\circ}\leq\Delta\theta\leq150^{\circ}$
\item $\Delta\theta=0^{\circ}\quad\leftrightarrow\quad$no debond!
\end{itemize}
\end{column}
...
\end{itemize}
\begin{itemize}[label=$\searrow$]
\item Mode I and Mode II SIF and ERR not defined for \emph{open crack}
\item ERR over finite distance
\item FEM + LEFM (VCCT)
\end{itemize}
\begin{itemize}[label=\ding{212}]
...
\scriptsize
\begin{itemize}[label=\ding{212}]
\item Total ERR does not depend on element size at the crack tip in any configuration of the crack.
\item For the \emph{close crack}, Mode II ERR does not depend on element size, as it is equal to the total ERR.
\item For the \emph{open crack}, Mode I and Mode II ERR depend logarithmically on element size.
\item For the \emph{open crack}, Mode I and Mode II ERR are not bounded.
\end{itemize}
\begin{alertblock}{\centering\scriptsize\bf It seems reasonable to conclude that...}
...a criterion based on the reduction of the error between successive iterations is inadequate to identify the optimal element size. The FEM model has to be validated with respect to external data (experimental, analytical or numerical from a different method).
\end{alertblock}
\end{frame}
...
\subsection{Analysis of initiation: main features}

\begin{frame}
\frametitle{\vspace{0.2cm}\small Analysis of initiation: main features}
\centering
\vspace{-0.25cm}
\begin{alertblock}{\small\centering NO DEBOND!}
\centering
\begin{figure}
\centering
...
\scriptsize
\begin{itemize}[label=\ding{212}]
\item No stress-based initiation criterion is assumed
\item Profiles of stresses in the matrix at the interface are analyzed
\item Where does the maximum occur? How fast does the stress decrease from the peak value?
\item Effect of thickness of UD/$90^{\circ}$ ply? Effect of the presence of $0^{\circ}$ ply?
\end{itemize}
\end{frame}

\subsection{Stress profiles in the matrix at the interface}

\begin{frame}
\frametitle{\vspace{0.2cm}\small Stress profiles in the matrix at the interface}
\vspace{-.75cm}
\centering
\begin{columns}[c]
\centering
...
\vspace{-.75cm}
\centering
\begin{alertblock}{\centering\scriptsize\bf Similar results for:}
\centering$\sigma_{LHS,2D}$ - $\sigma_{LHS,3D}$\\$\sigma_{LMPS,2D}$ - $\sigma_{LMPS,3D}$
\end{alertblock}
\end{column}
\end{columns}
\vspace{-0.4cm}
...
\begin{frame}
\frametitle{\vspace{0.2cm}\small Debond interaction: horizontal (loading) direction}
\vspace{-.75cm}
\centering
\begin{columns}[c]
\centering
...
\begin{frame}
\frametitle{\vspace{0.2cm}\small Effect of the presence of $0^{\circ}$ ply: comparison with equivalent BC}
\vspace{-.75cm}
\centering
\begin{columns}[c]
\centering
...
\end{equation*}
\begin{equation*}
\text{With input }\ R_{f}=1\mu m,\bar{\varepsilon}_{x}=1\%\longrightarrow G_{Ic}\in\left[2,4.5\right]\nicefrac{J}{m^{2}}
\end{equation*}
\begin{equation*}
\text{Experimentally }R_{f}\sim10\mu m,\bar{\varepsilon}_{x}\sim0.4\%\xrightarrow{rescaling}G_{Ic}\in\left[3.2,7.2\right]\nicefrac{J}{m^{2}}
\end{equation*}
\end{frame}
...
\scriptsize
\begin{itemize}[label=\ding{212}]
\item The presence of the $0^{\circ}$ layer results in a sizeable effect on debond ERR only in the case of a 1-fiber-row $90^{\circ}$ layer, i.e. $0^{\circ}$ layer extremely close to the debond.
\item If there is just one row of fully bonded fibers between the debond and the $0^{\circ}$ layer, the effect of the latter becomes almost negligible.
\item Debond-debond interaction along the vertical direction has signicant influence on ERR, but the presence of just 2 fully bonded fibers in between drastically reduces its importance.
...
\vspace{0.5cm}
\LARGE
\textbf{Thank you for listening today!}\\\vspace{0.1pt}
\end{frame}
