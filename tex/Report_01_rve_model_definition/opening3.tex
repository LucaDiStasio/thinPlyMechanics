...
\begin{scope}[style=axes]
  \draw[->] (-1.5*\l,-1.25*\cornerlabel) -- (\xD-2*\l*\sinthreepsi+0.25*\l,-1.25*\cornerlabel) node[right] {$i$};
  \draw[->] (-1.2*\l,-1.5*\cornerlabel) -- (-1.2*\l,\meshfacone*\Rf+2*\translfac*\meshfacone*\Rf) node[above] {$j$};
\end{scope}
